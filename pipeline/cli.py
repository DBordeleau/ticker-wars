...
def run_refresh_live_prices(
    *,
    ...
    regular_snapshots = [
        row for row in result.snapshots if row.get("market_state") == "regular"
    ]
...
def run_check_live_prices(
    *,
    ...
    return _report_live_price_health(
        snapshots,
...
def _parse_ticker_arg(value: str | None) -> tuple[str, ...] | None:
    if value is None:
        return None
...
def _build_live_price_fetch_event(
    *,
...
def _report_live_price_health(
    ...
    *,
    ...
    fresh_rows = [
        ...
        if _parse_timestamp(row.get("as_of")) >= stale_cutoff
        and _parse_timestamp(row.get("stale_after")) >= current
    ]
    ...
    newest_as_of = max((_parse_timestamp(row.get("as_of")) for row in snapshots), default=None)
    oldest_as_of = min((_parse_timestamp(row.get("as_of")) for row in snapshots), default=None)
