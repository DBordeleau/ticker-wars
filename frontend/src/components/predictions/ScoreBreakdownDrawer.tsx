      ...
      {prediction && score ? (
        <Stack gap="md">
          ...
          <Text size="sm" className="score-breakdown-explanation">
            You were {formatPercent(score.absolute_pct_error, 2)} away from the final close, so this
            landed in {verdictLabel}. The stock moved {directionName(score.actual_direction)} and you
            predicted {directionName(score.predicted_direction)}, so this was a {directionHit ? "direction hit" : "direction miss"}.
          </Text>
        </Stack>
      ) : (
